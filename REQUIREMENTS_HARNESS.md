...
| FLH-V-007 | The harness shall validate `cad-cli` compatibility, schema-contract conformance, and designer library availability as part of preflight and integration coverage. | The system depends on stable structured contracts and a consistent modeling environment. | Implemented — includes explicit checks for venv-local designer libs and cad shim shadowing. |
