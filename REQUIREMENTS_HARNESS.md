...
| FLH-F-007 | The normal design-review mode shall assess the latest candidate against the normalized spec, rendered outputs, deterministic inspections, and at most one optional user-provided reference image. | These are the minimum inputs needed to close the loop without ground-truth geometry. | Implemented |
| FLH-F-008 | The Reviewer shall be able to request further revision from the CAD Designer when the current candidate is not acceptable. | The review loop must be able to drive iteration. | Implemented |
...
| FLH-F-014 | The harness shall support developer eval runs over datasets with known ground-truth geometry. | Developer evals are a first-class system capability. | Implemented — eval datasets now use `case_id`, `prompt`, explicit `ground_truth_step`, and optional `reference_image`; `formloop eval run` accepts a dataset directory or direct `cases.jsonl`. |
| FLH-F-015 | The harness shall combine deterministic eval metrics with structured LLM judge outputs. | Both deterministic evidence and higher-level assessment are required. | Implemented — batch reports include deterministic metrics, judge outputs, and aggregate run/delivery/artifact/compare/judge rates. |
...
| FLH-NF-003 | The harness shall support reproducible eval execution suitable for before-and-after comparison. | The system should grow with a repeatable eval culture. | Implemented — eval runs persist dataset path, worker count, reference-image mode, effective profile/model/reasoning, effective role runtimes, per-case status stages, and aggregate rates. |
...
| FLH-NF-009 | The harness shall note and avoid wasteful repeated model or render work when behavior changes would materially increase cost. | Cost discipline is part of the operating model. | Partial — revision loop is bounded by `max_revisions` and short-circuits on pass; research fan-out is bounded by `max_research_topics` and each topic is one Responses request with `max_tool_calls=1`; CAD Designer source authoring is bounded by `max_cad_designer_turns=20`; eval case execution is bounded by `--workers` defaulting to 5 and does not auto-retry failed cases; the harness performs one final authoritative `cad build` per source instead of retrying source authoring inside the build-debug loop; CAD Designer tool traces record per-tool counts and arguments. No per-token metering yet. |
...
| FLH-V-008 | Eval validation shall confirm per-case artifacts, deterministic metrics, judge outputs, and aggregate reporting outputs. | Eval quality needs concrete proof at both per-case and batch levels. | Partial — runner + report emit per-case artifacts, deterministic metrics, judge outputs, case error records, and aggregate stage rates; a full batch run against `datasets/basic_shapes` is available via `formloop eval run --workers 5` but has not been captured as a fixture yet. |
...
- `formloop eval run` supports bounded parallel case execution with `--workers` defaulting to 5 and `--no-reference-images` to suppress optional case reference images for benchmark comparisons.
