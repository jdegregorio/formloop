...
| FLU-F-005 | The UI shall make the harness-produced rendered images, including the multi-view render sheet and individual persisted view images, available to the user as a secondary reference alongside the interactive GLB viewer. | Rendered images remain a useful reference for both human review and cross-checking the agent review signal. | Proposed |
...
| FLU-F-008 | The UI shall provide expandable access to tool-call history, specialist-call history, and intermediate traces, collapsed by default. | Traceability is required, but secondary details should stay out of the primary UI by default. | Proposed |
| FLU-F-009 | The UI shall surface intermediate progress updates by polling the harness for the current run snapshot plus append-only structured events that include LLM-generated milestone breadcrumbs. | The agreed v1 interface is polling-based and should explain what the harness is doing and why. | Proposed |
