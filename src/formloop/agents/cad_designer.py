...
2. If a model.py already exists, prefer surgical edits:
   ...
   - use ``apply_patch_to_model`` for targeted changes,
...
@function_tool
def apply_patch_to_model(ctx: RunContextWrapper[RunContext], patch: str) -> str:
    """Apply a unified diff patch to model.py in the working source directory."""
    model_path = _safe_model_path(ctx.context)
    if "model.py" not in patch:
        return "Patch rejected: include model.py in diff headers."
    proc = subprocess.run(
        ["git", "apply", "--no-index", "--whitespace=nowarn", "--recount", "-"],
        cwd=str(ctx.context.source_dir),
        input=patch,
        text=True,
        capture_output=True,
    )
    if proc.returncode != 0:
        return f"Patch failed ({proc.returncode}): {proc.stderr or proc.stdout}"
    updated = _read_source(model_path)
    return f"Patch applied to model.py ({len(updated.encode('utf-8'))} bytes)."
